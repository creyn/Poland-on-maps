print("Create map of Warsaw with Vistula and Leaflet")

install.packages("src/packages/POMUtils_1.1.tar.gz")
folders <- POMUtils::setup(
  script_folder = "src/15-Warsaw-detailed/",
  data_folder = "data/",
...
ggplot() +
	...
	geom_sf(data = warsaw_region, fill = alpha("#FFFFFF", 0), linewidth = 1) +
	geom_sf(data = vistula_boxed, color = "blue", linewidth = 1) +
	theme_void()
	# labs(title = "Vistula and Warsaw", x = "Longitude", y = "Latitude", fill = "Poland")

ggsave(paste(folders$final_map_folder, "15-Warsaw-detailed.png", sep = "/"))
...
# Rscript src/15-Warsaw-detailed/15-Warsaw-detailed.R
